/* =========================
   ...
   1) Fixed Power BI-like canvas 16: 9 default (1280×720), editable size
   ...
   5) Drag + Resize:
      ...
      - Clamped within canvas (all boundaries enforced)
   6) Format Pane (always visible):
      - Canvas settings when none selected: 
         • Editable canvas width/height
         • Canvas size dropdown preset
   ...
   10) Chart grid/axis lines removed for clean Power BI style
   11) Canvas boundary enforcement (all 4 sides)
   12) Screen fit - no overflow beyond viewport
========================= */
...
(() => {
  ...
  // -------------------------
  // Canvas size (editable, default 16:9)
  // -------------------------
  let CANVAS_W = 1280;
  let CANVAS_H = 720;
  ...
  function updateCanvasSize(w, h) {
    CANVAS_W = w;
    CANVAS_H = h;
    document. documentElement.style.setProperty("--canvasW", `${CANVAS_W}px`);
    document.documentElement.style.setProperty("--canvasH", `${CANVAS_H}px`);
    if (statusPill) statusPill.textContent = `Canvas: ${CANVAS_W} × ${CANVAS_H}`;
    
    // Clamp all visuals to new canvas size
    state.order.forEach(id => {
      const v = state.visuals.get(id);
      if (v) {
        v.x = clamp(v.x, 0, Math.max(0, CANVAS_W - v. w));
        v.y = clamp(v.y, 0, Math. max(0, CANVAS_H - v.h));
        v.w = clamp(v.w, 220, CANVAS_W);
        v.h = clamp(v.h, 170, CANVAS_H);
        applyVisualRect(v);
      }
    });
  }

  updateCanvasSize(CANVAS_W, CANVAS_H);
    // -------------------------
  ...
  const defaultTheme = {
    ...
    chart: {
      ...
      grid: "rgba(255,255,255,0.00)" // No grid lines
    }
  };
  ...
  // -------------------------
  // Visual registry (picker) - with NEW visuals
  // -------------------------
  const VISUALS = [
    {
      category: "Charts",
    ...
    },
    {
      ...
      items:  [
        ...
        { type: "multiCard", name: "Multi-row Card", icon: "📋" },
      ]
    },
    {
      category: "Text & Media",
  ...
  // Theme → Chart. js common options (NO grid/axis lines - Power BI clean style)
  ...
  function addVisual(type){
    ...
    // Ensure visual fits within canvas
    const fitW = Math.min(size. w, CANVAS_W);
    const fitH = Math.min(size.h, CANVAS_H);

    const offset = (state.order.length * 18) % 140;
    const x = clamp(40 + offset, 0, CANVAS_W - fitW);
    const y = clamp(40 + offset, 0, CANVAS_H - fitH);
    ...
    const v = {
      ...
      textContent: type === "textBox" ? "Enter your text here..." : "",
      textFontSize: 14,
      textColor: "#e8ecf2",
  ...
  function removeVisual(id){
    ...
    // -------------------------
  // Drag & Resize (smooth, overlap allowed, boundary enforced)
  // -------------------------
  let dragCtx = null;
  ...
  function onMove(e){
    ...
    if (dragCtx. mode === "drag") {
      // Enforce all 4 boundaries
      const maxX = CANVAS_W - v.w;
      const maxY = CANVAS_H - v.h;
      v.x = clamp(dragCtx.origX + dx, 0, Math.max(0, maxX));
      v.y = clamp(dragCtx. origY + dy, 0, Math. max(0, maxY));
      applyVisualRect(v);
    } else {
      const minW = 220;
      const minH = 170;
      ...
      // East resize - enforce right boundary
      if (hnd. includes("e")) {
        const newW = dragCtx.origW + dx;
        w = clamp(newW, minW, CANVAS_W - x);
      }

      // South resize - enforce bottom boundary
      if (hnd.includes("s")) {
        const newH = dragCtx.origH + dy;
        h = clamp(newH, minH, CANVAS_H - y);
      }

      // West resize - enforce left boundary
      if (hnd.includes("w")) {
      ...
      // North resize - enforce top boundary
      if (hnd.includes("n")) {
      ...
      x = clamp(x, 0, CANVAS_W - minW);
      y = clamp(y, 0, CANVAS_H - minH);
  ...
  function applyVisualRect(v){
    ...
    // -------------------------
  // Render visual content (Chart.js + prototypes + new visuals)
  // -------------------------
  function renderVisualContent(v){
    ...
    if (v.type === "textBox") {
      ...
      host.style.backgroundColor = v. textBgColor || "transparent";
      
      const textarea = document.createElement("textarea");
      textarea.className = "textBoxContent";
      textarea. value = v.textContent || "";
      textarea.style.fontSize = `${v.textFontSize || 14}px`;
      textarea.style.color = v.textColor || "#e8ecf2";
      textarea. style.fontWeight = v.textBold ? "bold" : "normal";
      textarea.style. textAlign = v.textAlign || "left";
    ...
    if (v.type === "kpi") {
      ...
      const value = retail.Sales[11]; // Latest value
      const prevValue = retail.Sales[10];
      const change = ((value - prevValue) / prevValue * 100).toFixed(1);
      const isUp = change >= 0;
      
      host.innerHTML = `
        <div class="kpiValue" style="color: ${v.series[0]?.color || '#22c55e'}">${value}K</div>
        <div class="kpiLabel">${escapeHtml(v.title)}</div>
        <div class="kpiTrend ${isUp ? 'up' : 'down'}">${isUp ? '▲' : '▼'} ${Math.abs(change)}% vs prev month</div>
      `;
      body.appendChild(host);
      return;
    }
    ...
    if (v.type === "card") {
      ...
      const value = retail.Sales. reduce((a, b) => a + b, 0);
      
      host.innerHTML = `
        <div class="cardValue" style="color: ${v.series[0]?.color || '#e8ecf2'}">${value}K</div>
        <div class="cardLabel">${escapeHtml(v. title)}</div>
      `;
      body.appendChild(host);
      return;
    }
    ...
    if (v.type === "multiCard") {
      const host = document.createElement("div");
      host.className = "multiCardHost";
      
      const metrics = [
        { label: "Total Sales", value: `${retail.Sales.reduce((a, b) => a + b, 0)}K` },
        { label: "Total Profit", value: `${retail. Profit.reduce((a, b) => a + b, 0)}K` },
        { label: "Total Orders", value: `${retail.Orders.reduce((a, b) => a + b, 0)}` },
        { label:  "Avg Monthly Sales", value: `${Math.round(retail. Sales.reduce((a, b) => a + b, 0) / 12)}K` }
      ];
      
      metrics.forEach((m, i) => {
        const row = document.createElement("div");
        row.className = "multiCardRow";
        row.innerHTML = `
          <div class="multiCardLabel">${escapeHtml(m.label)}</div>
          <div class="multiCardValue" style="color: ${paletteColor(i)}">${m.value}</div>
        `;
        ...
        return {
          config: {
            ...
            options:  {
              ...
              scales: {} // No scales for pie/donut
            }
          }
        };
      }
  ...
  // Ribbon prototype (SVG) - no grid lines
  function makeRibbonPrototype(v){
    ...
    // No grid lines - removed grid path
  ...
  function applyThemeToSingleVisual(v){
    if (v.type === "treemap" || v.type === "ribbon" || v.type === "kpi" || v.type === "card" || v.type === "multiCard" || v.type === "textBox" || v.type === "image") {
      renderVisualContent(v);
      return;
    }
    if (! v.chart) return;
    ...
    if (v.chart.options?.scales?.x?.grid) {
      v.chart.options.scales. x.grid.display = false;
      v.chart.options.scales.x. grid.drawBorder = false;
    }
    if (v.chart. options?.scales?.y?.grid) {
      v.chart. options.scales.y.grid.display = false;
      v.chart.options. scales.y.grid.drawBorder = false;
    }
  ...
  function renderFormatPane(){
    ...
    if (! selected) {
      // Canvas settings view with editable size
      const presetOptions = CANVAS_PRESETS. map(p => 
        `<option value="${p. w}x${p. h}" ${(CANVAS_W === p.w && CANVAS_H === p.h) ? 'selected' :  ''}>${p.label}</option>`
      ).join('') + `<option value="custom" ${! CANVAS_PRESETS.some(p => p.w === CANVAS_W && p.h === CANVAS_H) ? 'selected' :  ''}>Custom</option>`;
        ...
        <div class="fSection">
          ...
          <div class="row">
            <div class="field">
              ...
              <input class="input" id="canvasWidthInput" type="number" value="${CANVAS_W}" min="400" max="3840" />
            </div>
            <div class="field">
              <div class="label">Canvas height</div>
              <input class="input" id="canvasHeightInput" type="number" value="${CANVAS_H}" min="300" max="2160" />
            </div>
          </div>
          ...
          <div class="fSmall" style="margin-top:8px;">
            Upload format: <code>{ "backgroundColor": ".. .", "backgroundImage": ".. .", "opacity": 0.9 }</code>
          </div>
        </div>
      ...
      // Canvas size preset dropdown
      document.getElementById("canvasSizePreset").onchange = (e) => {
      ...
      document.getElementById("applyCanvasSizeBtn").onclick = () => {
        const w = parseInt(document.getElementById("canvasWidthInput").value) || 1280;
        const h = parseInt(document. getElementById("canvasHeightInput").value) || 720;
        updateCanvasSize(clamp(w, 400, 3840), clamp(h, 300, 2160));
    ...
    // Visual settings view - continues in next part
    renderVisualFormatPane(selected);
  }
    function renderVisualFormatPane(v) {
    const posHtml = `
      <div class="fSection">
        <div class="fHeader">
        ...
        <div class="row">
          <div class="field">
            ...
            <input class="input" id="fmtX" type="number" value="${escapeAttr(String(v.x))}" />
          </div>
          <div class="field">
            <div class="label">Y</div>
            <input class="input" id="fmtY" type="number" value="${escapeAttr(String(v. y))}" />
          </div>
        </div>
        ...
        <div class="row">
          <div class="field">
            ...
            <input class="input" id="fmtW" type="number" value="${escapeAttr(String(v.w))}" />
          </div>
          <div class="field">
            <div class="label">Height</div>
            <input class="input" id="fmtH" type="number" value="${escapeAttr(String(v. h))}" />
          </div>
        </div>
    ...
    if (v. type === "image") {
      ...
      colorsHtml = `
        <div class="fSection">
          <div class="fHeader">
            <div class="fHeaderTitle">Text Formatting</div>
          </div>
          <div class="textFormatRow">
            <button class="textFormatBtn ${v.textBold ? 'active' : ''}" id="textBoldBtn" title="Bold"><b>B</b></button>
            <button class="textFormatBtn ${v.textAlign === 'left' ? 'active' : ''}" id="textAlignLeft" title="Align Left">⫷</button>
            <button class="textFormatBtn ${v.textAlign === 'center' ? 'active' : ''}" id="textAlignCenter" title="Align Center">☰</button>
            <button class="textFormatBtn ${v.textAlign === 'right' ? 'active' : ''}" id="textAlignRight" title="Align Right">⫸</button>
          </div>
          <div class="row">
            <div class="field">
              <div class="label">Font size</div>
              <input class="input" id="textFontSize" type="number" value="${v.textFontSize || 14}" min="8" max="72" />
            </div>
            <div class="field">
              <div class="label">Text color</div>
              <input class="colorInput" id="textColorInput" type="color" value="${ensureHex(v.textColor || '#e8ecf2')}" />
            </div>
          </div>
          <div class="row one">
            <div class="field">
              <div class="label">Background color</div>
              <input class="input" id="textBgColorInput" value="${escapeAttr(v.textBgColor || 'transparent')}" placeholder="transparent or #hex" />
            </div>
          </div>
          <div class="smallBtnRow">
            <button class="btn" id="applyTextFormatBtn">Apply Formatting</button>
          </div>
        </div>
      `;
    } else if (v.type === "kpi" || v.type === "card" || v.type === "multiCard") {
      colorsHtml = `
        <div class="fSection">
          <div class="fHeader">
            <div class="fHeaderTitle">Value Color</div>
          </div>
          <div class="colorRow">
            <input class="input" value="Primary Color" disabled />
            <input class="colorInput" id="kpiColorInput" type="color" value="${ensureHex(v. series[0]?.color || paletteColor(0))}" />
          </div>
          <div class="smallBtnRow">
            <button class="btn" id="applyKpiColorBtn">Apply Color</button>
          </div>
        </div>
      `;
    } else {
    ...
    document.getElementById("fmtTitle").oninput = (e) => {
      ...
      if (v.type === "kpi" || v. type === "card") renderVisualContent(v);
    };
    ...
    document.getElementById("applyPosBtn").onclick = () => {
      ...
      if (isFinite(nw)) v.w = clamp(nw, 220, CANVAS_W);
      if (isFinite(nh)) v.h = clamp(nh, 170, CANVAS_H);
    ...
    if (v.type === "textBox") {
      document.getElementById("textBoldBtn").onclick = () => {
        v. textBold = ! v.textBold;
        renderVisualContent(v);
        renderFormatPane();
      };
      document.getElementById("textAlignLeft").onclick = () => { v.textAlign = "left"; renderVisualContent(v); renderFormatPane(); };
      document.getElementById("textAlignCenter").onclick = () => { v.textAlign = "center"; renderVisualContent(v); renderFormatPane(); };
      document.getElementById("textAlignRight").onclick = () => { v.textAlign = "right"; renderVisualContent(v); renderFormatPane(); };
      
      document.getElementById("applyTextFormatBtn").onclick = () => {
        v. textFontSize = parseInt(document.getElementById("textFontSize").value) || 14;
        v.textColor = document.getElementById("textColorInput").value;
        v.textBgColor = document. getElementById("textBgColorInput").value.trim() || "transparent";
        renderVisualContent(v);
    ...
    // KPI/Card color
    if (v.type === "kpi" || v. type === "card" || v.type === "multiCard") {
      document.getElementById("applyKpiColorBtn").onclick = () => {
        const color = document.getElementById("kpiColorInput").value;
        if (! v.series. length) v.series = [{ key: "Value", name: "Value", color, overrideColor: true }];
        else { v.series[0].color = color; v.series[0].overrideColor = true; }
        renderVisualContent(v);
    ...
    // Chart series colors
    if (! ["image", "textBox", "kpi", "card", "multiCard"].includes(v. type)) {
      v.series.forEach((s, idx) => {
        ...
        if (nameEl) nameEl.oninput = (e) => { s.name = e. target.value; applyThemeToSingleVisual(v); };
        if (colEl) colEl.oninput = (e) => { s.color = e.target.value; s.overrideColor = true; applyThemeToSingleVisual(v); };
      });

      const resetBtn = document.getElementById("resetVisualColorsBtn");
      if (resetBtn) {
        resetBtn. onclick = () => {
          v.series.forEach((s, i) => { s.color = paletteColor(i); s.overrideColor = false; });
          applyThemeToSingleVisual(v);
          renderFormatPane();
  ...
  function renderSeriesEditor(v){
    if (!v.series || v.series.length === 0) return `<div class="fSmall">No series for this visual. </div>`;
  ...
  function ensureHex(c){
    if (! c) return "#3b82f6";
    if (c.startsWith("#")) { if (c.length === 4) return "#" + c. slice(1).split("").map(x=>x+x).join(""); return c.slice(0,7); }
  ...
  // Theme import
  if (importThemeBtn) importThemeBtn. addEventListener("click", () => { importThemeInput.value = ""; importThemeInput.click(); });
  if (importThemeInput) importThemeInput. addEventListener("change", async () => {
    const f = importThemeInput.files?.[0]; if (!f) return;
    try { const t = await readJsonFile(f); theme = normalizeTheme(t); applyThemeEverywhere(); showToast("Theme import successful"); }
    catch { showToast("Invalid theme JSON"); }
  });

  function normalizeTheme(t){
    const out = structuredClone(defaultTheme); if (! t || typeof t !== "object") return out;
    ...
    out.chart = out.chart || {};
    if (t. chart && typeof t. chart === "object") out.chart = { ...out.chart, ... t.chart };
    out.chart.grid = "rgba(255,255,255,0.00)"; // Force no grid
    return out;
  }

  // Dashboard save/load
  if (downloadDashboardBtn) downloadDashboardBtn.addEventListener("click", () => { downloadObjectAsJson(serializeDashboard(), "dashboard.json"); showToast("Dashboard downloaded"); });
  if (uploadDashboardBtn) uploadDashboardBtn.addEventListener("click", () => { uploadDashboardInput.value = ""; uploadDashboardInput.click(); });
  if (uploadDashboardInput) uploadDashboardInput.addEventListener("change", async () => {
    const f = uploadDashboardInput.files?.[0]; if (!f) return;
    const loadIt = async () => { try { const obj = await readJsonFile(f); await loadDashboard(obj); showToast("Dashboard loaded"); } catch { showToast("Invalid dashboard file"); } finally { uploadDashboardInput.value = ""; } };
    if (! isDefaultState()) openDiscardModal(loadIt); else loadIt();
  });

  function openDiscardModal(onConfirm){
    if (! modalBackdrop || isDefaultState()) { onConfirm?. (); return; }
    modalBackdrop.hidden = false; modalBackdrop. style.display = "flex"; modalBackdrop. setAttribute("aria-hidden", "false");
    const close = () => { modalBackdrop.hidden = true; modalBackdrop.style.display = "none"; modalBackdrop.setAttribute("aria-hidden", "true"); };
    modalCancelBtn.onclick = () => close();
    modalConfirmBtn.onclick = async () => { close(); await onConfirm(); };
  }

  function serializeDashboard(){
    const visuals = state.order.map(id => {
      const v = state.visuals. get(id); if (!v) return null;
      return { id:  v.id, type: v.type, title: v.title, x: v.x, y: v.y, w: v. w, h: v.h,
        series: (v.series || []).map(s => ({ key: s.key, name: s.name, color: s. color, overrideColor: !!s.overrideColor })),
        imageDataUrl: v. type === "image" ?  (v.imageDataUrl || "") : undefined,
        textContent: v.type === "textBox" ?  v.textContent :  undefined,
        textFontSize: v. type === "textBox" ? v.textFontSize : undefined,
        textColor: v.type === "textBox" ?  v.textColor :  undefined,
        textBgColor: v.type === "textBox" ? v. textBgColor :  undefined,
        textBold: v.type === "textBox" ?  v.textBold : undefined,
        textAlign: v. type === "textBox" ? v.textAlign : undefined
      };
    }).filter(Boolean);
    return { version: 1, canvas: { width: CANVAS_W, height:  CANVAS_H, background: canvasBg }, theme, visuals };
  }

  async function loadDashboard(obj){
    clearAllVisuals();
    if (obj?. canvas?.width && obj?.canvas?.height) updateCanvasSize(obj.canvas.width, obj.canvas.height);
    if (obj?.canvas?.background) canvasBg = normalizeCanvasBg(obj.canvas. background); else canvasBg = structuredClone(sampleCanvasBg);
    applyCanvasBackground();
    theme = normalizeTheme(obj?. theme || defaultTheme); applyThemeEverywhere();
    const visuals = Array.isArray(obj?.visuals) ? obj.visuals :  [];
    visuals.forEach(vs => {
      const id = String(vs.id || `vis${state.nextId++}`);
      const v = { id, type: vs. type, title: vs.title || defaultTitleFor(vs.type),
        x: clamp(Number(vs.x)||40, 0, CANVAS_W-220), y: clamp(Number(vs. y)||40, 0, CANVAS_H-170),
        w: clamp(Number(vs.w)||DEFAULT_SIZES[vs.type]?. w||520, 220, CANVAS_W), h: clamp(Number(vs.h)||DEFAULT_SIZES[vs.type]?.h||300, 170, CANVAS_H),
        series: Array.isArray(vs.series) ? vs.series. map((s,i)=>({ key: s.key || s.name || `S${i+1}`, name: s.name || s.key || `Series ${i+1}`, color: s.color || paletteColor(i), overrideColor: !!s.overrideColor })) : buildDefaultSeries(vs. type),
        imageDataUrl: vs. type === "image" ? (vs.imageDataUrl || "") : undefined, chart: null,
        textContent: vs.textContent || "", textFontSize:  vs.textFontSize || 14, textColor:  vs.textColor || "#e8ecf2",
        textBgColor: vs.textBgColor || "transparent", textBold: vs.textBold || false, textAlign: vs.textAlign || "left"
      };
      v.series?. forEach((s,i)=>{ if (! s.overrideColor) s.color = paletteColor(i); });
      state.visuals.set(id, v); state.order.push(id);
      const el = createVisualElement(v); canvas.appendChild(el); renderVisualContent(v);
    });
    updateZOrder(); setSelected(null); state.nextId = Math.max(state.nextId, state.order.length + 1);
  }

  function clearAllVisuals(){
    state.order. forEach(id => { const v = state.visuals.get(id); if (v?.chart) { try { v.chart. destroy(); } catch {} } });
    state.visuals.clear(); state.order = []; state.selectedId = null;
    Array.from(canvas. querySelectorAll(". visual")).forEach(el => el.remove());
  }

  // Sample downloads
  if (downloadSampleThemeBtn) downloadSampleThemeBtn.addEventListener("click", () => { downloadObjectAsJson(sampleTheme, "sample-theme.json"); showToast("Sample theme downloaded"); });
  if (downloadSampleCanvasBgBtn) downloadSampleCanvasBgBtn.addEventListener("click", () => { downloadObjectAsJson(sampleCanvasBg, "sample-canvas-background.json"); showToast("Sample canvas background downloaded"); });
